...
async def test_gp_device_tables_fresh_db(tmp_path):
    """A fresh DB at the current schema version has gp_devices and gp_proxy_table tables."""
    db = tmp_path / "test.db"
    ...
    assert "gp_proxy_table_v16" in tables


async def test_v15_to_v16_migration(test_db):
    """Migration from v15 adds GP tables (empty) and preserves existing device rows."""
    ...
    with sqlite3.connect(test_db_v15) as conn:
        ...
        cur.execute("SELECT COUNT(*) FROM gp_proxy_table_v16")
        assert cur.fetchone()[0] == 0
